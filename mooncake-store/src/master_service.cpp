...
std::optional<uint32_t> ExtractNoFNamespaceId(const std::string& endpoint) {
    ...
    std::smatch match;
    if (!std::regex_search(endpoint, match, ns_regex)) {
        return std::nullopt;
    }
    try {
        return static_cast<uint32_t>(std::stoul(match[2].str()));
    } catch (const std::exception&) {
        return std::nullopt;
    }
}

std::string BuildNoFEndpointWithNamespace(const std::string& endpoint,
                                          uint32_t nsid) {
    static const std::regex ns_regex(R"((^|\s)ns:\d+(\s|$))");
    std::string replacement = "$1ns:" + std::to_string(nsid) + "$2";
    if (std::regex_search(endpoint, ns_regex)) {
        return std::regex_replace(endpoint, ns_regex, replacement,
...
uint64_t MasterService::ReleaseExpiredDiscardedReplicas(
    ...
    discarded_replicas_.remove_if(
        [&now, &released_cnt, replica_type](const DiscardedReplicas& item) {
            const bool expired = item.isExpired(now);
            if (expired && item.replicaSize() > 0 && item.hasTargetReplica(replica_type)) {
                released_cnt++;
            }
            return expired;
        });
...
void MasterService::BatchEvict(double evict_ratio_target,
    ...
    // second pass.
    uint64_t released_discarded_cnt = ReleaseExpiredDiscardedReplicas(now, ReplicaType::MEMORY);
    ...
    VLOG(1) << "action=evict_mem_replicas" << ", evicted_count=" << evicted_count
            << ", total_freed_size=" << total_freed_size;
}

void MasterService::NoFBatchEvict(double nof_evict_ratio_target,
                               double nof_evict_ratio_lowerbound) {
    if (nof_evict_ratio_target < nof_evict_ratio_lowerbound) {
        LOG(ERROR) << "nof_evict_ratio_target=" << nof_evict_ratio_target
                   << ", nof_evict_ratio_lowerbound=" << nof_evict_ratio_lowerbound
                   << ", error=invalid_params";
        nof_evict_ratio_lowerbound = nof_evict_ratio_target;
    }
    ...
    for (size_t i = 0; i < metadata_shards_.size(); i++) {
        ...
        // ideally how many object should be evicted in this shard
        const long ideal_evict_num =
            std::ceil(object_count * nof_evict_ratio_target) - evicted_count;
        ...
        if (ideal_evict_num > 0 && !candidates.empty()) {
            ...
            while (it != shard.metadata.end()) {
                ...
                if (it->second.lease_timeout <= target_timeout) {
                    // Evict this object
                    total_freed_size +=
                        it->second.size * it->second.GetNoFReplicaCount();
                    it->second.EraseReplica(
                        ReplicaType::NOF_SSD);  // Erase nof replicas
                    if (it->second.IsValid() == false) {
                        it = shard.metadata.erase(it);
                    } else {
                        ++it;
    ...
    // Try releasing discarded replicas before we decide whether to do the
    // second pass.
    uint64_t released_discarded_cnt = ReleaseExpiredDiscardedReplicas(now, ReplicaType::NOF_SSD);

    // The ideal number of objects to evict in the second pass
    long target_evict_num = std::ceil(object_count * nof_evict_ratio_lowerbound) -
                            evicted_count - released_discarded_cnt;
    // The actual number of objects we can evict in the second pass
    target_evict_num =
        std::min(target_evict_num,
    ...
    if (target_evict_num > 0) {
        ...
        if (target_evict_num <= static_cast<long>(no_pin_objects.size())) {
                ...
                while (it != shard.metadata.end() && target_evict_num > 0) {
                    ...
                    if (it->second.lease_timeout <= target_timeout &&
                        ...
                        total_freed_size +=
                            it->second.size * it->second.GetNoFReplicaCount();
                        it->second.EraseReplica(
                            ReplicaType::NOF_SSD);  // Erase memory replicas
                        if (it->second.IsValid() == false) {
                            it = shard.metadata.erase(it);
                        } else {
                            ++it;
            ...
            LOG(ERROR) << "Error in second pass eviction: target_evict_num="
                       ...
                       << ", evicted_count=" << evicted_count
                       << ", object_count=" << object_count
                       << ", evict_ratio_target=" << nof_evict_ratio_target
                       << ", evict_ratio_lowerbound=" << nof_evict_ratio_lowerbound;
        }
    }
    ...
    if (evicted_count > 0 || released_discarded_cnt > 0) {
        ...
        MasterMetricManager::instance().inc_eviction_success(evicted_count,
                                                             total_freed_size);
        MasterMetricManager::instance().inc_nof_eviction_success(evicted_count,
                                                             total_freed_size);
    } else {
        if (object_count == 0) {
            // No objects to evict, no need to check again
    ...
    VLOG(1) << "action=evict_nof_replicas" << ", evicted_count=" << evicted_count
            << ", total_freed_size=" << total_freed_size;
}
...
void MasterService::HandleNoFNamespaceAttributeChanges(
    const std::vector<MountedNoFSegmentSnapshot>& mounted_segments) {
#ifndef USE_NOF
    (void)mounted_segments;
#else
    std::vector<std::string> changed_ctrlrs =
        SpdkWrapper::GetInstance().ConsumeNamespaceAttributeChangedControllers();
    if (changed_ctrlrs.empty()) {
        return;
    }

    std::map<std::string, std::vector<MountedNoFSegmentSnapshot>>
        segments_by_ctrlr;
    for (const auto& snapshot : mounted_segments) {
        ...
        if (!ctrlr_key.empty()) {
            segments_by_ctrlr[ctrlr_key].push_back(snapshot);
        }
    }

    for (const auto& ctrlr_key : changed_ctrlrs) {
        auto ctrlr_it = segments_by_ctrlr.find(ctrlr_key);
        if (ctrlr_it == segments_by_ctrlr.end()) {
            LOG(WARNING) << "ctrlr_key=" << ctrlr_key
                         << ", action=nof_namespace_change_skip"
...
void MasterService::TryMountAddedNoFNamespaces(
    const std::string& ctrlr_key,
    const std::vector<MountedNoFSegmentSnapshot>& ctrlr_segments) {
#ifndef USE_NOF
    (void)ctrlr_key;
    ...
    std::set<uint32_t> mounted_nsids;
    for (const auto& snapshot : ctrlr_segments) {
        auto nsid = ExtractNoFNamespaceId(snapshot.segment.te_endpoint);
        if (nsid.has_value()) {
            mounted_nsids.insert(*nsid);
        }
    }
    ...
    for (uint32_t nsid : active_nsids) {
        if (mounted_nsids.contains(nsid)) {
            continue;
        }

        std::string endpoint = BuildNoFEndpointWithNamespace(
            template_snapshot.segment.te_endpoint, nsid);
        std::string error_reason;
        if (!ProbeNoFSegment(endpoint, &error_reason)) {
            LOG(WARNING) << "ctrlr_key=" << ctrlr_key
                         << ", endpoint=" << endpoint
                         << ", action=nof_namespace_add_rejected"
                         << ", reason=" << error_reason;
            if (IsNoFControllerErrorReason(error_reason)) {
                SpdkWrapper::GetInstance().InvalidateNofController(endpoint);
            } else {
                SpdkWrapper::GetInstance().CloseNofSegment(endpoint);
            }
            continue;
        }

        nof_seg_handle *seg_handle =
            SpdkWrapper::GetInstance().OpenNofSegment(endpoint);
        if (!seg_handle) {
            LOG(WARNING) << "ctrlr_key=" << ctrlr_key
                         << ", endpoint=" << endpoint
                         << ", action=nof_namespace_add_rejected"
                         << ", reason=open_after_probe_failed";
            continue;
        }
        ...
        if (capacity == 0) {
            LOG(WARNING) << "ctrlr_key=" << ctrlr_key
                         << ", endpoint=" << endpoint
                         << ", action=nof_namespace_add_rejected"
                         << ", reason=invalid_namespace_capacity";
            SpdkWrapper::GetInstance().CloseNofSegment(endpoint);
            continue;
        }
        ...
        auto mount_result =
            MountNoFSegment(new_segment, template_snapshot.client_id);
        if (!mount_result) {
            LOG(WARNING) << "ctrlr_key=" << ctrlr_key
                         << ", endpoint=" << endpoint
                         << ", action=nof_namespace_add_mount_failed"
                         << ", reason=" << mount_result.error();
            SpdkWrapper::GetInstance().CloseNofSegment(endpoint);
            continue;
        }

        LOG(INFO) << "ctrlr_key=" << ctrlr_key
                  << ", endpoint=" << endpoint
                  << ", nsid=" << nsid
                  << ", capacity=" << capacity
                  << ", action=nof_namespace_added";
    }
#endif
}
...
void MasterService::NofHeartbeatThreadFunc() {
    ...
    while (nof_heartbeat_running_) {
        ...
        std::vector<MountedNoFSegmentSnapshot> mounted_segments;
        nof_segment_manager_.GetMountedSegmentsSnapshot(mounted_segments);
        HandleNoFNamespaceAttributeChanges(mounted_segments);
